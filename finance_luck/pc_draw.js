...
//      $('#lottery-wrap').html($('#lottery-wrap').html());
    ...
    function startLottery() {
        ...
        // 开奖次数已达上线
        // var awards = local_handle.get('award_4');
        // if(awards){
        //     if(JSON.parse(awards).length == 9) {
        //         isLock = false;
        //         alert('抽奖次数已达到上限，请清除数据后重新抽奖.')
        //         return
        //     }
        // }
...
var time=0;
    ...
    function stopLottery() {
        ...
        var lottery_initial2 = local_handle.get('lottery_initial');
        var lottery_datas2 = local_handle.get('lottery_datas');
        var award_history = local_handle.get('award_history');
        var newData = [];
        if(award_history) {
            newData = JSON.parse(lottery_datas2)
        }else {
            newData = JSON.parse(lottery_initial2)
        }
        ...
        for( item in newData ) {
            ...
            if(newData[item].state) {
                indexList.push(Number(item) + 1)
            }
        }
		...
		if(lottery_size>4){
            ...
            console.log(lottery_size)
            if(lottery_size === 74) {
                var stop_index= "";
                for( item in newData ) {
                
                    if(newData[item].nameen == 'avatar8') {
                        stop_index = Number(item) - 4
                    }
                }
                if(!stop_index) {
                    stop_index =  indexList[Math.floor(Math.random() * indexList.length)]-5;
                }
            }else{
                var stop_index =  indexList[Math.floor(Math.random() * indexList.length)]-5;
...
//		context.drawImage(back_img, 0, 0);
	...
	//		=name_zh.substring(0,1)+" "+name_zh.substring(1,2)+" "+name_zh.substring(2,3)+" "+name_zh.substring(3,4);
			context.fillText(name_str, 78, 410);
...
//                  this.music.pause();
		...
		var lottery_btn_fun=function(){
            ...
            if (award_log['award0'+cur_lottery] <= 0) {
                alert('该奖项抽奖次数已达上限，请清除数据后重新开始哦 ^_^！');
                return;
            }
